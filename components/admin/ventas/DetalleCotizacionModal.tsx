...
import { Download, FileText, Loader2, Plus, Trash2, CreditCard } from 'lucide-react'
import { Modal } from '@/components/ui/Modal'
import { format } from 'date-fns'
import { es } from 'date-fns/locale'
import { actualizarCotizacion, registrarPagoCotizacion, eliminarPagoCotizacion } from '@/app/admin/_actions/ventas'
...
export function DetalleCotizacionModal({ cotizacion }: DetalleCotizacionModalProps) {
  ...
  const saldoRestante = Math.max(0, cotizacion.total - totalPagado)
  ...
  const handleGuardarAbono = async (e: React.FormEvent) => {
    ...
    const res = await registrarPagoCotizacion(cotizacion.id, formData)
    if (res.ok) {
      setMontoAbono('')
      setRefAbono('')
      setMostrarFormAbono(false)
  ...
  const handleEliminarAbono = async (pagoId: string) => {
    ...
    const res = await eliminarPagoCotizacion(pagoId, cotizacion.id)
    if (res.ok) {
      await cargarPagos()
    } else {
        ...
        <form onSubmit={handleUpdate} className="space-y-4">
          ...
          <div className="bg-[var(--bg-raise)] border border-[var(--line-soft)] rounded-xl p-4 space-y-3">
            <div className="flex justify-between items-center border-b border-[var(--line-soft)] pb-2">
              <h4 className="font-serif italic text-[var(--blue)] text-md flex items-center gap-2">
                <CreditCard className="w-4 h-4" /> Historial de Abonos
              </h4>
            ...
            {mostrarFormAbono && (
              <div className="bg-[var(--bg-elevated)] border border-[var(--line-soft)] rounded-lg p-3 space-y-3">
                <h5 className="text-xs font-semibold text-[var(--fg)]">Nuevo Abono</h5>
                {errorAbono && (
            ...
            {cargandoPagos ? (
              <div className="text-center py-4 text-xs text-[var(--fg-dim)]">Cargando abonos...</div>
            ) : pagos.length > 0 ? (
              ...
              <div className="text-center py-4 text-xs text-[var(--fg-dim)] italic bg-[var(--bg-elevated)] border border-[var(--line-soft)] rounded-lg">
                No se han registrado abonos para esta cotización.
              </div>
            )}
          </div>
          ...
          <div className="bg-[var(--bg-raise)] border border-[var(--line-soft)] rounded-xl p-4">
            ...
            <div className="flex justify-between items-center mb-2 border-t border-[var(--line-soft)] pt-2">
              <span className="text-[var(--fg-dim)] text-sm">Total Cotización</span>
              <span className="text-[var(--fg)] font-medium">{formatCOP(cotizacion.total)}</span>
            </div>
            <div className="flex justify-between items-center mb-2">
